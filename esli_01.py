...
def calculate_scores(scored_responses: dict) -> pd.DataFrame:
    ...
    try:
        ...
        # --- 3. 계산 항목 정의 ---
        # 컬럼명을 Gradio 질문 텍스트에서 계산에 사용될 원래 컬럼명으로 매핑합니다.
        # 이 매핑은 수동으로 생성해야 합니다. (기존 파일의 헤더 기반)
        # 예: '마음대로 일이 되지 않으면 불안하다' -> '감정과 행동 패턴 (1/7) [마음대로 일이 되지 않으면 불안하다]'
        # 이 과정이 복잡하므로, 계산 로직에서 직접 질문 텍스트를 사용하도록 수정합니다.
        ...
        calculations = {
            ...
            '직접적': {'cols': ['좋은 성적을 얻으면 용돈을 주시거나 좋은 선물을 사주니까', '공부 안하면 어른들에게 잔소리 들으니까', '공부하는 것은 정말 싫지만 선생님이나 부모님이 하라고 하니까', '부모님이나 선생님이 바라시는 대학에 가기위해', '좋은 성적을 받지 못하면 용돈이 줄거나 자유시간이 줄어서', '시험 성적이 떨어지면 부모님께 혼나는 것이 싫어서', '선생님이나 부모님이 공부하라고 한 분량을 맞춰놓아야 하니까'], 'multiplier': 7},
            '관계적': {'cols': ['남들 다 하는 공부 나만 안하면 불안해서', '부모님이나 선생님이 기대하는 것을 만족시켜드리기 위해', '학생은 당연히 공부를 해야 하니까', '공부잘하면 다른 아이들이 나를 함부로 대하지 못하니까', '공부잘하면 원하는 대학에 입학할 수 있으니까', '공부잘해서 좋은 성적을 얻으면 다른 사람들이 칭찬해주니까', '가족들에게 모범이 되는 모습을 보여주어야 하니까'], 'multiplier': 7},
            ...
            '목표': {'cols': ['어떤 일을 하기 전에 항상 목표세워 시작한다', '특별히 되고 싶은 직업이나 장래 희망이 없다', '미래에 성공한 나의 모습을 상상하면 마음이 설렌다', '누군가가 목표를 정해주고 나는 그대로 시키는대로만 했으면 좋겠다', '공부할 때는 별로 목표수립은 필요없다', '흥미 있는 대학 학과나 직업에 대해 이것 저것 찾아본 적이 있다', '목표가 있으면 부담스러워서 목표를 세우지 않고 공부하는 편이다'], 'multiplier': 7},
            '계획': {'cols': ['공부 계획을 위한 다이어리나 계획표를 사용한다', '따로 공부계획을 세우지 않고 그날 그날 공부한다', '계획을 세워도 지키지 않는 경우가 더 많다', '하루에 얼만큼 공부할 수 있을지 잘 모르기 때문에 계획을 미리 짜는 것은 불가능하다', '오늘 해야 할 공부가 계획되어 있다', '단원평가나 학교시험의 범위와 일정에 맞춰 계획을 세워 공부한다', '계획은 어차피 바뀌므로 굳이 세울 필요가 없다'], 'multiplier': 7},
            '실천': {'cols': ['공부하다보면 쉽게 피곤해져서 계속 공부하기 어렵다', '공부하기로 마음 먹고 나서도 한참 지나야 겨우 공부를 시작한다', 'TV나 주변 소리에도 크게 신경쓰지 않고 공부할 수 있다', '오늘 할 일은 미루지 않고 끝내려고 노력한다', '게임이나 친구들과 놀이 때문에 계획한 공부시간을 놓치는 경우가 많다', '공부하기 어려운 과목도 해야 할 분량은 빼먹지 않고 공부한다', '책상 앞에 앉아 있지만 집중해서 공부한 시간은 얼마 되지 않는다'], 'multiplier': 7},
            ...
            '스트레스': {'cols': ['마음대로 일이 되지 않으면 불안하다', '좋지 않은 일이 생기면 배가 아픈 경우가 많다', '작은 일도 다른 사람들이 어떻게 생각할지 신경이 많이 쓰인다', '하고 싶어하는 일을 하기도 전에 잘못될 걱정을 많이 한다', '능력이 부족하다고 생각해서 어떤 일을 중간에 그만둔 적이 있다', '운이 나빠서 자꾸 일이 잘못 되는 것 같다'], 'multiplier': 6},
            '효능감': {'cols': ['다른 친구들에 비해 머리가 많이 나쁜것 같다', '내가 공부한 만큼 또는 그 이상 결과를 얻고 있다', '아무리 노력해도 좋은 성적을 받을 수 없을 것 같다', '머리가 좋은 편이어서 친구들보다 쉽게 공부한다', '어려운 문제라도 충분히 시간이 주어지면 스스로 풀어낼 자신이 있다', '혼자 문제를 푸는 것보다 다른 사람에게 물어보는 것이 낫다'], 'multiplier': 6},
            '친구': {'cols': ['학교에서 은근 따돌림 받는 것 같다', '학교에서 먼저 반겨주고 이야기 건네주는 친구가 있다', '학교 끝나고 친구들과 만나서 놀기도 한다', '친구들에게 내가 먼저 말을 걸기가 어렵다', '주변 친구들은 대부분 나와 사이가 좋지 않다', '친구들과 함께 노는 일은 정말 즐겁다'], 'multiplier': 6},
            '가정': {'cols': ['나에 관한 결정에서 부모님은 항상 내 의견 묻고 결정한다', '부모님과 밥 먹는 것이 힘들다', '부모님이 형제자매나 사촌과 비교하는 말을 자주한다', '모든것을 마음대로 결정하는 부모님 때문에 답답하다', '갑자기 깜짝 놀랄 정도로 부모님께서 화를 내는 경우가 자주 있다', '가족 중에 나를 이해해주는 사람이 있어서 고민을 이야기 할 수 있다'], 'multiplier': 6},
            '학교': {'cols': ['학교에서 배우는 것이 나에게 많은 도움이 될 것 같다', '학교 선생님에게 꾸중보다 칭찬을 더 듣는다', '담임선생님이 내가 나쁜 학생이라고 불친절하다', '방학때 너무 심심해서 차라리 학교가고 싶다는 생각을 한 적이 있다', '학교에 가면 나도 모르게 무섭거나 짜증나서 별로 가고 싶지 않다', '학교에 가는 것은 재미있는 일이다'], 'multiplier': 6},
            '수면': {'cols': ['밤10시만 되도 잠이 와서 공부하기 어렵다', '학교 수업시간에 자는 경우가 많다', '단원평가나 학교시험을 위해서는 평소보다 잠을 줄여 공부하는 편이다', '하루 평균 10시간 이상 자는 것 같다', '혼자 공부하다 잠이 오면 잠을 깨는 나만의 방법이 있다'], 'multiplier': 5},
            '집중력': {'cols': ['공부하려고 앉으면 10분도 안되서 딴 생각에 빠진다', '제대로 공부에 집중하려면 최소 10분이상 준비할 시간이 필요하다', '공부하다가 나도 모르게 시간이 훌쩍 지나간 경우가 많다', '공부시작하면 마칠 때까지 거의 공부만 한다', 'TV나 주변 소리에도 크게 신경쓰지 않고 공부할 수 있다'], 'multiplier': 5},
            'TV': {'cols': ['숙제를 하다가도 꼭 봐야하는 TV프로그램이 있다', '하루에 1~2시간 이상 TV를 본다', 'TV드라마나 어린이 프로그램 한 두편 정도 보는 것은 크게 상관없다', '친구들과 대화하는 거의 모든 주제는 TV프로그램과 관련된 것이다', '내가 좋아하는 TV프로그램을 놓치면 궁금해서 다른 일을 할 수가 없다'], 'multiplier': 5},
            ...
            '스마트': {'cols': ['핸드폰이나 스마트기기가 없어도 내 생활에 큰 영향은 없다', '문자나 인터넷, 게임 등을 위해 하루 1시간 이상 핸드폰을 한다', '핸드폰이 거의 1분 간격으로 카톡 알림이 울린다', '친구들과 톡을 주고 받지 못하면 불안하다', '스마트폰 데이터가 다 떨어져서 사용못하면 매우 답답하다'], 'multiplier': 5},
        }
